...
// 绘制剧情区
void redraw_story(void* ctx, int leftWileft_widthdth, int rows);
// 清空矩形区域（像素单位）
...
// 新增：绘制场景列表（位于按钮区顶部）
void draw_scene_list(void* ctx, int left_width, int cols, int rows, int selected, int startY, int btn_width);
